...
namespace NS_Communion {
  ...
  u_char* CommunicatorApplication::writeInPGM()
  {
	...
	for(unsigned long y = 0; y < respMap->map.info.height; y++) {
	    for(unsigned long x = 0; x < respMap->map.info.width; x++) {
	    	unsigned long i = x + (respMap->map.info.height - y - 1) * respMap->map.info.width;
			if (respMap->map.data[i] == 0) { //occ [0,0.1)
				out[count++] = 254;
			} else if (respMap->map.data[i] == +100) { //occ (0.65,1]
	            out[count++] = 000;
	        } else { //occ [0.1,0.65]
	            out[count++] = 205;
	        }
  ...
  void CommunicatorApplication::saveMapInPGM()
  {
	mapDataFile = "/home/nico/git/SeNavigation/Debug/map.pgm";
	FILE* out = fopen(mapDataFile.c_str(), "w");
	if (!out)
	{
	  NS_NaviCommon::console.warning("Couldn't save map file to %s", mapDataFile.c_str());
	}
	fprintf(out, "P5\n# CREATOR: Map_generator.cpp %.3f m/pix\n%d %d\n255\n",
	              respMap->map.info.resolution, respMap->map.info.width, respMap->map.info.height);
	...
	fclose(out);
	NS_NaviCommon::console.message("write PGM finish, path: %s", mapDataFile.c_str());
  ...
  void CommunicatorApplication::onReceive(CommData* message)
  {
    ...
    if (message != NULL) {
    	if (message->reason == COMMUNICATION_DATA_REASON_MAP_SIZE) {
        	...
        	if (respMap == NULL)
        		NS_NaviCommon::console.message("you have to get map size before request Map data!");
        	...
        	NS_NaviCommon::console.message("receive payload: %s>>>>>>", response->payload);
            ...
            */
    		...
    		NS_NaviCommon::console.message("receive message: COMMUNICATION_DATA_REASON_MAP_META!");
    	}
